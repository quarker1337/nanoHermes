...
def _clear_auxiliary_provider_health():
    """Provider-priority tests should not inherit unhealthy-provider state."""
    try:
        auxiliary_client = importlib.import_module("agent.auxiliary_client")
    except Exception:
        yield
        return
    auxiliary_client._aux_unhealthy_until.clear()
    auxiliary_client._aux_unhealthy_logged_at.clear()
    yield
    auxiliary_client._aux_unhealthy_until.clear()
    auxiliary_client._aux_unhealthy_logged_at.clear()
...
def _make_agent(monkeypatch, provider, api_mode="chat_completions", base_url="https://openrouter.ai/api/v1", model=None):
    monkeypatch.setattr("hermes_runtime.run_agent.get_tool_definitions", lambda **kw: _tool_defs("web_search", "terminal"))
    monkeypatch.setattr("hermes_runtime.run_agent.check_toolset_requirements", lambda: {})
    monkeypatch.setattr("hermes_runtime.run_agent.OpenAI", _FakeOpenAI)
    kwargs = dict(
        api_key="test-key",
        base_url=base_url,
        provider=provider,
...
class TestAuxiliaryClientProviderPriority:
    ...
    def test_openrouter_always_wins(self, monkeypatch):
        ...
        # Nano preserves the live main-model override for auxiliary auto mode.
    ...
    def test_nous_when_no_openrouter(self, monkeypatch):
        ...
        with patch("agent.auxiliary_client._read_nous_auth", return_value=nous_auth), \
            ...
            client, model = get_text_auxiliary_client(
                main_runtime={"provider": "nous", "model": "test-model"}
            )
        # Nano preserves the live main-model override for auxiliary auto mode.
        assert model == "test-model"
