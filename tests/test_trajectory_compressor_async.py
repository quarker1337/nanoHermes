...
class TestAsyncClientLazyCreation:
    """runtime/hermes_runtime/trajectory_compressor.py — _get_async_client()"""

    def test_async_client_none_after_init(self):
        """async_client should be None after __init__ (not eagerly created)."""
        from hermes_runtime.trajectory_compressor import TrajectoryCompressor
...
class TestSourceLineVerification:
    ...
    def _read_file() -> str:
        ...
        with open(os.path.join(base, "runtime/hermes_runtime/trajectory_compressor.py")) as f:
            return f.read()
...
async def test_generate_summary_async_kimi_omits_temperature():
    ...
    from hermes_runtime.trajectory_compressor import CompressionConfig, TrajectoryCompressor, TrajectoryMetrics
