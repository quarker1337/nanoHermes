...
class TestBrowserConsoleToolsetWiring:
    ...
    def test_in_legacy_toolset_map(self):
        from hermes_runtime.model_tools import _LEGACY_TOOLSET_MAP
...
class TestBrowserVisionConfig:
    ...
    def test_browser_vision_uses_configured_temperature_and_timeout(self, tmp_path):
        ...
        with (
            patch("hermes_runtime.hermes_constants.get_hermes_dir", return_value=shots_dir),
...
class TestDogfoodSkill:
    ...
    def _skill_dir(self):
        # Use the actual repo bundled skills dir (not temp)
        self.skill_dir = os.path.join(
            os.path.dirname(__file__), "..", "..", "resources", "skills", "dogfood"
        )
