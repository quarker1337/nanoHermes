...
def test_dockerfile_installs_tui_dependencies(dockerfile_text):
    ...
    assert "apps/tui/package-lock.json" in dockerfile_text
    assert "apps/tui/packages/hermes-ink/" in dockerfile_text
    assert any(
        "apps/tui" in step and "npm" in step and (" install" in step or " ci" in step)
