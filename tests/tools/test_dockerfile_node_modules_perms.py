...
`npm install` in /opt/hermes/apps/tui. that install fails with EACCES unless
...
def test_dockerfile_chowns_runtime_node_modules_to_hermes_user() -> None:
    ...
    # /opt/hermes/apps/dashboard is intentionally excluded: it is build-time only,
    # because HERMES_WEB_DIST points at hermes_cli/web_dist for runtime.
    for required_path in ("/opt/hermes/apps/tui", "/opt/hermes/node_modules"):
