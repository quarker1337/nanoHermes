...
class TestCmdUpdatePip:
    """Regression tests for PyPI/pip-install update flows."""
    ...
    @patch("shutil.which", return_value="/home/wayne/.local/bin/uv")
    @patch("subprocess.run")
    def test_update_pip_branch_flag_rewrites_github_archive_direct_url(
        self, mock_run, _mock_which, monkeypatch
    ):
        """Runtime NanoHermes installs can update to a named feature branch.

        The default runtime installer records a PEP 610 direct URL like
        ``.../archive/refs/heads/main.tar.gz``. Without branch-aware rewrite,
        ``hermes update --branch sync/...`` still upgraded the main tarball,
        leaving package-manager fixes on feature branches unavailable to
        minimal testers.
        """
        from hermes_cli import main as hm

        source = "https://github.com/quarker1337/nanoHermes/archive/refs/heads/main.tar.gz"
        branch = "sync/upstream-selective-20260603"
        expected = f"https://github.com/quarker1337/nanoHermes/archive/refs/heads/{branch}.tar.gz"
        ...
        hm._cmd_update_pip(SimpleNamespace(branch=branch))
    ...
    def test_update_pip_uses_uv_system_outside_virtualenv(
        ...
        """Bare/non-venv pip installs must pass --system to uv pip.

        Regression for the real failure:
        ``uv pip install --upgrade hermes-agent`` exits with
        "No virtual environment found" when Hermes is not running inside a
        venv. ``--system`` tells uv to target the active interpreter instead.
        """
        from hermes_cli import main as hm
    ...
    @patch("shutil.which", return_value="/usr/bin/uv")
    @patch("subprocess.run")
    def test_update_pip_uses_uv_tool_upgrade_for_uv_tool_install(
        self, mock_run, _mock_which, mock_args, monkeypatch
    ):
        from hermes_cli import main as hm
        ...
        mock_run.return_value = subprocess.CompletedProcess([], 0, stdout="", stderr="")
        monkeypatch.delenv("VIRTUAL_ENV", raising=False)
        monkeypatch.setattr(hm.sys, "prefix", "/home/wayne/.local/share/uv/tools/hermes-agent")
        monkeypatch.setattr(hm.sys, "base_prefix", "/usr")
        monkeypatch.setattr(
            hm.sys,
            "executable",
    ...
    @patch("subprocess.run")
    def test_update_pip_uses_pipx_upgrade_for_pipx_managed_install(
        ...
        assert mock_run.call_count == 1
        assert mock_run.call_args.args[0] == ["/usr/bin/pipx", "upgrade", "hermes-agent"]
        assert "env" not in mock_run.call_args.kwargs
