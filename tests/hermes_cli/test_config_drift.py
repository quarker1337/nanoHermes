...
def test_delegation_default_toolsets_removed_from_cli_config():
    ...
    Guards against accidental re-introduction in runtime/hermes_runtime/cli.py's CLI_CONFIG default
    ...
    the user's ~/.hermes/config.yaml over the defaults (runtime/hermes_runtime/cli.py:359-366).
    ...
    (runtime/hermes_runtime/cli.py:76) — before any autouse fixture can fire. Source inspection
    ...
    assert '"default_toolsets"' not in source, (
        ...
        "Do not re-add it to runtime/hermes_runtime/cli.py's CLI_CONFIG default dict; "
