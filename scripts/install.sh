...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  --branch NAME  Git branch to install (implies --source unless --runtime was explicit; default: main)"
...
#!/usr/bin/env bash
# NanoHermes launcher generated by scripts/install.sh.
# Hermes Agent launcher: safe for hermes uninstall to remove.
unset PYTHONPATH
unset PYTHONHOME
exec "$HERMES_BIN" "\$@"
EOF
...
SOUL_EOF
    ...
    if [ "$NO_SKILLS" = true ]; then
        ...
        printf '%s
'             "This profile opted out of bundled-skill seeding (installed with --no-skills)."             "Delete this file to re-enable sync on the next 'hermes update'."             > "$HERMES_HOME/.no-bundled-skills" 2>/dev/null || true
