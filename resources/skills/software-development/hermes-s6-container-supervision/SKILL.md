...
- Understanding why the container's CMD is `/opt/hermes/infra/docker/main-wrapper.sh` and how leading-dash args reach the user's program
...
If you're just running the Hermes Agent and want to use Docker, see `docs/site/docs/user-guide/docker.md` instead.
...
│   ├── 01-hermes-setup                ← infra/docker/stage2-hook.sh
│   │   ├── UID/GID remap
│   │   ├── chown /opt/data
...
│   └── ...
│
└── CMD ("main program")               ← /opt/hermes/infra/docker/main-wrapper.sh
...
| Path | Role |
|---|---|
| `Dockerfile` | s6-overlay install + cont-init.d wiring + `ENTRYPOINT ["/init", "/opt/hermes/infra/docker/main-wrapper.sh"]` |
| `infra/docker/stage2-hook.sh` | The "old entrypoint logic" — UID remap, chown, seed, skills sync. Runs as cont-init.d/01-hermes-setup. |
| `infra/docker/cont-init.d/02-reconcile-profiles` | Calls `hermes_cli.container_boot` on every boot to restore profile gateway slots from the persistent volume. |
| `infra/docker/main-wrapper.sh` | The container's CMD. Routes user args, drops to hermes via `s6-setuidgid`, exec's the chosen program. |
| `infra/docker/s6-rc.d/main-hermes/run` | No-op `sleep infinity` — slot exists so the s6-rc user bundle is valid; main hermes runs as the CMD, not as a supervised service. |
| `infra/docker/s6-rc.d/dashboard/run` | Conditional service — `exec sleep infinity` unless `HERMES_DASHBOARD` is truthy. |
| `infra/docker/entrypoint.sh` | Back-compat shim that `exec`s the stage2 hook. External scripts that hard-coded the old entrypoint path still work. |
...
1. Create `infra/docker/s6-rc.d/<name>/type` with `longrun\n` and `infra/docker/s6-rc.d/<name>/run` (use `#!/command/with-contenv sh` + `# shellcheck shell=sh`).
2. Drop to hermes via `s6-setuidgid hermes` at the top of run (unless you specifically need root).
3. Create empty `infra/docker/s6-rc.d/<name>/dependencies.d/base` so it waits for the base bundle.
4. Create empty `infra/docker/s6-rc.d/user/contents.d/<name>` so it joins the user bundle.
5. The `COPY infra/docker/s6-rc.d/` in the Dockerfile picks it up automatically — no other changes.
