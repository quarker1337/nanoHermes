...
def _submit_fal_video_request(endpoint: str, arguments: Dict[str, Any]):
    ...
    if managed_gateway is None:
        if hasattr(_fal_client, "submit"):
            return _fal_client.submit(endpoint, arguments=arguments, headers=request_headers)
        if hasattr(_fal_client, "subscribe"):
            result = _fal_client.subscribe(endpoint, arguments=arguments, with_logs=False)

            class _ImmediateFalHandle:
                def get(self):
                    return result

            return _ImmediateFalHandle()
        raise AttributeError("fal_client has neither submit nor subscribe")
