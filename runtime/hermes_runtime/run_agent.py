...
# `mock.patch("hermes_runtime.run_agent.<X>")`, `from hermes_runtime.run_agent import <X>` in production
...
from agent.process_bootstrap import (
    ...
    _SafeWriter,  # noqa: F401  # re-exported for tests that `from hermes_runtime.run_agent import _SafeWriter`
...
from agent.prompt_builder import (  # noqa: F401  # re-exported via _ra() / mock.patch("hermes_runtime.run_agent.<name>") / from hermes_runtime.run_agent import <name>
...
from agent.tool_dispatch_helpers import (
    ...
    _extract_parallel_scope_path,  # noqa: F401  # re-exported for tests that `from hermes_runtime.run_agent import _extract_parallel_scope_path`
    _paths_overlap,  # noqa: F401  # re-exported for tests that `from hermes_runtime.run_agent import _paths_overlap`
    _is_multimodal_tool_result,
    _multimodal_text_summary,
    _append_subdir_hint_to_multimodal,  # noqa: F401  # re-exported for tests that `from hermes_runtime.run_agent import _append_subdir_hint_to_multimodal`
    _extract_file_mutation_targets,
    _extract_error_preview,
    _trajectory_normalize_msg,  # noqa: F401  # re-exported for tests that `from hermes_runtime.run_agent import _trajectory_normalize_msg`
)
...
class AIAgent:
    ...
    def _decorate_xai_entitlement_error(detail: str) -> str:
        """Return xAI OAuth entitlement details without Hermes editorializing.

        xAI's error bodies already carry the actionable subscription/quota URL
        (``https://grok.com/?_s=usage``).  Older Hermes builds appended a hint
        claiming X Premium+ did not include xAI API access; that became stale
        when xAI announced Grok/Hermes support for X Premium tiers.  Keep the
        provider body verbatim so users see xAI's current wording.
        """
        return detail
