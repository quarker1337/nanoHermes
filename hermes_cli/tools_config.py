...
def _visible_providers(
    ...
    *,
    ...
    Nous-managed Tool Gateway rows (``managed_nous_feature``) are shown only
    when the current account is entitled for that managed capability. Logged
    out / unentitled users should not see a row that cannot be activated.
    """
    features = get_nous_subscription_features(config, force_fresh=force_fresh)
    ...
    for provider in cat.get("providers", []):
        ...
        if managed_feature:
            if not (acct and acct.logged_in):
                continue
            coverage = MANAGED_FEATURE_COVERAGE_CATEGORY.get(managed_feature)
            entitled = (
                acct.tool_gateway_entitled_for(coverage)
                if coverage
        ...
        # (pure pre-auth UX) is hidden until the user is logged in.
        if (
            provider.get("requires_nous_auth")
            and not managed_feature
    ...
    # Keep entitled Nous-managed rows prominent while preserving relative order
    # within managed vs self-hosted/local groups.
    visible.sort(key=lambda p: 0 if p.get("managed_nous_feature") else 1)
...
def _reconfigure_provider(
    ...
    *,
    ...
    if managed_feature and provider.get("requires_nous_auth"):
        from hermes_cli.nous_subscription import (
            MANAGED_FEATURE_COVERAGE_CATEGORY,
            ensure_nous_portal_access,
        )
