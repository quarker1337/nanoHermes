...
class _EventChannelsProxy:
    """Compatibility proxy for tests/tools that inspected the old global map.

    Runtime state still lives on ``app.state`` so the asyncio lock is scoped to
    the active ASGI event loop; this proxy exposes the current mapping without
    moving lock ownership back to module globals.
    """

    def _mapping(self) -> dict:
        channels, _lock = _get_event_state(app)
        return channels
...
_event_channels = _EventChannelsProxy()
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    """
    from hermes_runtime.hermes_constants import set_hermes_home_override, reset_hermes_home_override
