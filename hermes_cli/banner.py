...
def check_for_updates() -> Optional[int]:
    ...
    # Docker images update by pulling a new image, not by probing git/PyPI from
    # inside the container. Short-circuit before cache reads/writes so a
    # container never shows a stale or phantom "behind" badge.
    try:
        from hermes_cli.config import detect_install_method

        if detect_install_method(Path(__file__).parent.parent.resolve()) == "docker":
            return None
    except Exception:
        pass
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    """
    from hermes_runtime.model_tools import TOOLSET_REQUIREMENTS, check_tool_availability
