...
def cmd_postinstall(args):
    """One-shot bootstrap for pip users: stamp install + run setup."""
    ...
    print()
    print("Runtime dependencies are installed by NanoHermes packages, not base postinstall.")
...
def _model_flow_named_custom(config, provider_info):
    ...
    if models:
        ...
        try:
            if not sys.stdin.isatty():
                raise OSError("non-interactive stdin")
            from hermes_cli.curses_ui import curses_radiolist
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    The dashboard source lives under ``apps/dashboard/`` and the Vite build
    outputs to ``hermes_cli/web_dist/`` (per vite.config.ts
    outDir: "../../hermes_cli/web_dist"), NOT to ``apps/dashboard/dist/``.
...
def _cmd_gui_source_checkout_legacy(args: argparse.Namespace):
    """Build and launch the legacy source-checkout Electron desktop GUI.

    Kept for upstream reference only.  NanoHermes exposes the package-managed
    desktop launcher through the earlier ``cmd_gui`` definition, which delegates
    to ``_run_desktop()`` and resolves the workspace under HERMES_HOME.  Leaving
    this function named ``cmd_gui`` would override that Nano launcher during
    module import.
    """
    desktop_dir = PROJECT_ROOT / "apps" / "desktop"
    if not (desktop_dir / "package.json").exists():
...
def _update_node_dependencies() -> None:
    ...
    if (PROJECT_ROOT / "package.json").exists():
        # Upstream workspace layout: a root install would cover ALL workspaces,
        # including apps/desktop's Electron dependency. Keep desktop opt-in by
        # doing root-only first, then only the workspaces the CLI/TUI/web build
        # actually needs.
    ...
    # NanoHermes bucketed layout: there is intentionally no root Node
    # workspace. Install only concrete app packages that exist; do not run npm
    # in the repo root (it has no package.json and may create new root files).
    updated: list[str] = []
...
def _cmd_update_pip(args):
    ...
    from hermes_cli.managed_uv import resolve_uv, update_managed_uv

    # Pip/uv-tool/pipx installs should not bootstrap a managed uv as a side
    # effect of `hermes update`: this path is for updating the installed
    # package itself, not for source-checkout dependency management.  Prefer an
    # existing managed uv if present/current, then a user-provided uv on PATH;
    # otherwise fall back to pip (or fail clearly for uv-tool installs).
    uv = update_managed_uv() or resolve_uv() or shutil.which("uv")
    ...
    if is_uv_tool_install():
        ...
        if direct_url_target:
            ...
            # checks the already-installed package version and can no-op,
            # leaving runtime testers stuck on old updater/launcher code.
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    is_known_official_origin = bool(origin_url) and not is_fork
    ...
    try:
        ...
        if current_branch != branch:
            ...
            if is_known_official_origin and _clean_managed_worktree(git_cmd, PROJECT_ROOT):
                auto_stash_ref = None
            else:
...
def main():
    ...
    postinstall_parser = subparsers.add_parser(
        "postinstall",
        help="Finalize pip installs and run setup if needed",
        description="One-shot post-install for pip users. Stamps the install "
        "method and runs setup if needed. Runtime dependencies are installed "
        "through NanoHermes packages, not base postinstall.",
    )
