...
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    ...
    "hermes_runtime.hermes_state.db",
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    """Return the wrapper alias that launches *profile_name*, if one exists.

    Wrapper filenames are the commands users type. A profile can therefore be
    named ``steve`` while its wrapper is ``qiaobusi`` (``hermes -p steve``).
    ``hermes profile list`` should surface that actual command instead of
    assuming the wrapper has the same name as the profile.
    """
    ...
    for wrapper_path in sorted(wrapper_dir.iterdir(), key=lambda p: p.name):
        ...
        if not match:
            continue
        raw_profile = next((group for group in match.groups() if group), "")
        try:
            if normalize_profile_name(raw_profile) != target:
                continue
        except ValueError:
            continue
        alias = wrapper_path.stem if wrapper_path.suffix.lower() == ".bat" else wrapper_path.name
        matches.append(alias)

    if not matches:
        return None
    # Prefer a custom alias over the profile-name wrapper: it is the command the
    # user intentionally chose when they created the wrapper.
    return next((alias for alias in matches if alias != target), matches[0])
...
class ProfileInfo:
    ...
    def alias_name(self) -> Optional[str]:
        """Command name for this profile's wrapper, with platform suffix stripped."""
        if self.alias_path is None:
            return None
        return self.alias_path.stem if self.alias_path.suffix.lower() == ".bat" else self.alias_path.name
...
def list_profiles() -> List[ProfileInfo]:
    ...
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            ...
            if alias_name:
                alias_path = wrapper_dir / (f"{alias_name}.bat" if sys.platform == "win32" else alias_name)
            else:
                alias_path = None
