...
Outputs:
  docs/site/static/llms.txt        — short curated index of the docs, one link per page,
                                    grouped by section. Conforms to https://llmstxt.org.
  docs/site/static/llms-full.txt   — every `.md` file under `docs/site/docs/` concatenated,
...
The `/docs/` prefix is not a mistake — Docusaurus serves `docs/site/static/`
...
Called from `docs/site/scripts/prebuild.mjs` on every `npm run start` /
...
SECTIONS: list[tuple[str, list[tuple[str, str, str | None]]]] = [
    ...
    ("Reference", [
        ...
        ("reference/skills-catalog", "Source Skills Catalog", "Reference table of source skill definitions; NanoHermes activates skills through packages"),
...
def emit_llms_full() -> str:
    """Concatenate every doc under docs/site/docs/ into a single markdown file.
    ...
    def emit_file(rel: str) -> None:
        ...
        chunks.append(f"<!-- source: docs/site/docs/{path.relative_to(DOCS)} -->\n")
    ...
    for path in sorted(DOCS.rglob("*.md")):
        ...
        chunks.append(f"<!-- source: docs/site/docs/{rel} -->\n")
        chunks.append(f"# {title}\n\n")
        chunks.append(body.rstrip() + "\n\n---\n\n")
