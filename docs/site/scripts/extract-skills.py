#!/usr/bin/env python3
"""Extract skill metadata into docs/site/static/api/skills.json for the Skills Hub page.
...
2. The unified Hermes Skills Index at ``docs/site/static/api/skills-index.json``,
...
UNIFIED_INDEX_PATH = os.path.join(REPO_ROOT, "docs", "site", "static", "api", "skills-index.json")
...
OUTPUT = os.path.join(REPO_ROOT, "docs", "site", "static", "api", "skills.json")
META_OUTPUT = os.path.join(REPO_ROOT, "docs", "site", "static", "api", "skills-meta.json")
...
# docs/site/src/pages/skills/index.tsx.
...
# fallback. Used only when docs/site/static/api/skills-index.json is absent.
...
def _docs_page_path(rel_dir: str, source_label: str) -> str:
    ...
    Mirrors the slug logic in docs/site/scripts/generate-skill-docs.py:
...
def extract_unified_index_skills():
    """Read docs/site/static/api/skills-index.json — the canonical multi-source index.
