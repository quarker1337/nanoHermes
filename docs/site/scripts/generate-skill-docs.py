...
Each skill gets docs/site/docs/user-guide/skills/<source>/<category>/<skill-name>.md
where <source> is "bundled" or "optional".

Also regenerates:
- docs/site/docs/reference/skills-catalog.md
- docs/site/docs/reference/optional-skills-catalog.md
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        "---",
        "sidebar_position: 5",
        'title: "Source Skills Catalog"',
        'description: "Reference catalog of source skill definitions; NanoHermes installs skills through packages"',
        "---",
        "",
        "# Source Skills Catalog",
        "",
        "This reference lists source skill definitions generated from the repository. NanoHermes base installs zero skills by default; install capability profiles or skill packages from the registry to activate workflows.",
        "",
        "Package-managed and user-created skills live in your profile's `~/.hermes/skills/` tree. User edits are preserved; use `hermes pkg show <package>` to inspect what a package installs before enabling it.",
        "",
        "If a skill is missing from this list but present in the repo, the catalog is regenerated by `docs/site/scripts/generate-skill-docs.py`.",
        "",
    ]
...
def write_sidebar(entries):
    ...
    sidebar_path = REPO / "docs" / "site" / "sidebars.ts"
