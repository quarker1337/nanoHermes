...
:::warning
The default `messaging` extra intentionally does **not** install `discord.py[voice]`. `discord.py` 2.7.1 currently caps its voice extra to a vulnerable PyNaCl line, so NanoHermes keeps Discord text/bot support separate from Discord voice-channel dependencies until upstream supports patched PyNaCl.
:::
