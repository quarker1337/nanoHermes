...
1. Runs `/etc/cont-init.d/01-hermes-setup` (= `infra/docker/stage2-hook.sh`) as root: optional UID/GID remap, fixes volume ownership, seeds `.env` / `config.yaml` / `SOUL.md` on first boot, syncs bundled skills.
