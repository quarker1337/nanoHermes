...
On first launch Hermes installs the TUI's Node dependencies into `apps/tui/node_modules` (one-time, a few seconds). Subsequent launches are fast. If you pull a new Hermes version, the TUI bundle is rebuilt automatically when sources are newer than the dist.
...
```bash
export HERMES_TUI_DIR=/path/to/prebuilt/apps/tui
hermes --tui
```
