...
| `--no-skills` | Create an **empty** profile with zero preinstalled skills. Writes a `.no-skills` marker into the profile so future `hermes update` runs keep the zero-skill baseline, and refuses to combine with `--clone` / `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should install only explicit packages. |
