...
| `HERMES_BUNDLED_SKILLS` | Comma-separated legacy override for repo-provided skills loaded at startup. |
...
| `HERMES_TUI_DIR` | Path to a prebuilt `apps/tui/` directory (must contain `dist/entry.js` and populated `node_modules`). Used by distros and Nix to skip the first-launch `npm install`. |
