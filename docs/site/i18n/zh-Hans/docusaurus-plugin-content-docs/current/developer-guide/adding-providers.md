...
2. `hermes_cli/runtime_provider.py` 将其转换为运行时数据：
   ...
   - `api_key`
   - `source`
3. `runtime/hermes_runtime/run_agent.py` 使用 `api_mode` 决定如何构建和发送请求。
...
- `runtime/hermes_runtime/run_agent.py` 中用于请求构建、分发、用量提取、中断处理和响应规范化的分支
- 适配器测试
...
## 第 7 步：如果 provider 是原生的，添加适配器和 `runtime/hermes_runtime/run_agent.py` 支持

如果 provider 不是普通的 chat completions，将 provider 专属逻辑隔离在 `agent/<provider>_adapter.py` 中。

保持 `runtime/hermes_runtime/run_agent.py` 专注于编排。它应该调用适配器辅助函数，而不是在整个文件中内联构建 provider 请求载荷。
...
- 如有需要，转换工具 schema
- 将 provider 响应规范化为 `runtime/hermes_runtime/run_agent.py` 期望的格式
...
同时在 `runtime/hermes_runtime/run_agent.py` 中搜索 `self.client.`。任何假设标准 OpenAI 客户端存在的代码路径，在原生 provider 使用不同客户端对象或 `self.client = None` 时都可能中断。
...
- `tests/test_runtime/hermes_runtime/run_agent.py`
...
- [ ] 在 `runtime/hermes_runtime/run_agent.py` 中支持新的 `api_mode`
- [ ] 中断 / 重建路径正常工作
...
### 5. 原生 provider 分支隐藏在 `runtime/hermes_runtime/run_agent.py` 中
