...
`runtime/hermes_runtime/model_tools.py` 负责导入/发现工具模块，并构建供模型使用的 schema 列表。
...
当 `runtime/hermes_runtime/model_tools.py` 被导入时，会调用 `tools/registry.py` 中的 `discover_builtin_tools()`。该函数使用 AST 解析扫描所有 `tools/*.py` 文件，找出包含顶层 `registry.register()` 调用的模块，然后导入它们：
...
模型响应包含 tool_call
    ↓
runtime/hermes_runtime/run_agent.py agent 循环
    ↓
