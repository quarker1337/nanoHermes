...
```text
hermes-agent/
├── runtime/hermes_runtime/run_agent.py              # AIAgent — 核心对话循环（大文件）
├── runtime/hermes_runtime/cli.py                    # HermesCLI — 交互式终端 UI（大文件）
├── runtime/hermes_runtime/model_tools.py            # 工具发现、schema 收集、分发
├── runtime/hermes_runtime/toolsets.py               # 工具分组与平台预设
├── runtime/hermes_runtime/hermes_state.py           # 带 FTS5 的 SQLite 会话/状态数据库
├── hermes_constants.py       # HERMES_HOME、感知 profile 的路径
├── runtime/hermes_runtime/batch_runner.py           # 批量轨迹生成
│
├── agent/                    # Agent 内部模块
...
同步编排引擎（`runtime/hermes_runtime/run_agent.py` 中的 `AIAgent`）。负责 provider 选择、prompt 构建、工具执行、重试、回退、回调、压缩和持久化。支持三种 API 模式以适配不同 provider 后端。
...
tools/*.py  （每个文件在导入时调用 registry.register()）
       ↑
runtime/hermes_runtime/model_tools.py  （导入 tools/registry 并触发工具发现）
       ↑
