...
源文件：`agent/trajectory.py`、`runtime/hermes_runtime/run_agent.py`（搜索 `_save_trajectory`）、`runtime/hermes_runtime/batch_runner.py`
...
批量运行器（`runtime/hermes_runtime/batch_runner.py`）按批次写入自定义输出文件
...
### 批量运行器格式（来自 `runtime/hermes_runtime/batch_runner.py`）
