...
:::warning
默认的 `messaging` extra 有意不安装 `discord.py[voice]`。`discord.py` 2.7.1 当前把 voice extra 限制在存在漏洞的 PyNaCl 版本线，因此 NanoHermes 暂时只随 `messaging` 提供 Discord 文本/机器人支持，等上游支持已修复的 PyNaCl 后再恢复语音频道依赖。
:::
