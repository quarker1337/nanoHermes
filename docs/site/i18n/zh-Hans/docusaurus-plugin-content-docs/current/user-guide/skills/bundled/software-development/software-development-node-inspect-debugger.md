...
| 标签 | `debugging`, `nodejs`, `node-inspect`, `cdp`, `breakpoints`, `apps/tui` |
...
- Node 测试失败，需要查看中间状态
- apps/tui 崩溃或行为异常，需要在渲染前检查 React/Ink 状态
...
Hermes 专项说明：`chrome-remote-interface` 不在 `apps/tui/package.json` 中。如果不想污染项目，可将其安装到临时目录：
...
`apps/tui/package.json` 有 `npm run dev`（tsx --watch）。直接运行 tsx 并添加 `--inspect-brk`：
...
这些是 Python 进程，不是 Node——请使用 `python-debugpy` skill。只有 Node 部分（Ink UI、tui_gateway client、`apps/tui/` 下的 tsx-run 测试）使用本 skill。
