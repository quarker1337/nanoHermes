"""Compatibility shim for the relocated tui_gateway package.

The real package lives under ``runtime/tui_gateway`` so the repository root stays
small while legacy imports such as ``import tui_gateway`` and ``python -m tui_gateway.x``
...
_REAL_PACKAGE_DIR = Path(__file__).resolve().parent / "runtime" / "tui_gateway"
