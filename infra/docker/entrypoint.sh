#!/bin/sh
# s6-overlay shim. The real logic lives in infra/docker/stage2-hook.sh, invoked
# by /etc/cont-init.d/01-hermes-setup (installed by the Dockerfile). This
# file exists so external references to infra/docker/entrypoint.sh still work,
# but it's no longer the ENTRYPOINT — /init is.
#
# When called directly (e.g. by an old wrapper script that hard-coded
# infra/docker/entrypoint.sh as the container ENTRYPOINT, or by an external
...
echo "[hermes] WARNING: infra/docker/entrypoint.sh is a deprecated shim under " \
    "s6-overlay. The container's real ENTRYPOINT is /init + " \
    "main-wrapper.sh; this script only runs the stage2 cont-init hook " \
    "and does NOT exec the CMD. If you hard-coded infra/docker/entrypoint.sh " \
