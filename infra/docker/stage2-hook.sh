...
# Dockerfile. The shim at infra/docker/entrypoint.sh forwards to this script
# so external references to infra/docker/entrypoint.sh still work.
#
...
if [ "$needs_chown" = true ]; then
    ...
    #   - apps/tui: esbuild rebuilds dist/entry.js on every TUI launch (when
    #     the source mtime is newer than dist/ or when HERMES_TUI_FORCE_BUILD
    #     is set) and writes to apps/tui/dist/. Without this chown the new
