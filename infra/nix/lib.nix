# infra/nix/lib.nix — Shared helpers for nix stuff
{
  pkgs,
  npm-lockfile-fix,
  nodejs,
}:
{
  ...
  #
  # Usage:
  #   npm = hermesNpmLib.mkNpmPassthru { folder = "apps/tui"; attr = "tui"; pname = "hermes-tui"; };
  #   pkgs.buildNpmPackage (npm // { ... } # or:
  #   pkgs.buildNpmPackage ({ ... } // npm)
  mkNpmPassthru =
    {
      folder, # repo-relative folder with package.json, e.g. "apps/tui"
      attr, # flake package attr, e.g. "tui"
      pname, # e.g. "hermes-tui"
      nixFile ? "infra/nix/${attr}.nix", # defaults to infra/nix/<attr>.nix
    }:
    {
      inherit nodejs;
      patchPhase = ''
