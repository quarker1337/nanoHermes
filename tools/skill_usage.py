...
def _prune_builtins_enabled() -> bool:
    ...
    NanoHermes keeps bundled and hub-installed skills off-limits by default.
    The helper remains for compatibility with callers that display/configure
    the setting, but curation eligibility is still gated by provenance below.
    """
    try:
...
def list_agent_created_skill_names() -> List[str]:
    ...
    Only agent-authored/local skills marked in ``.usage.json`` via
    ``skill_manage(action="create")`` are returned. Bundled and hub-installed
    skills are upstream-owned and stay off-limits.
    """
    base = _skills_dir()
    if not base.exists():
        return []
    off_limits = _read_hub_installed_names() | _read_bundled_manifest_names()
    usage = load_usage()
...
def is_curation_eligible(skill_name: str) -> bool:
    ...
    Only agent/local skills are curation-eligible. Bundled and hub-installed
    skills are upstream-owned and must not be mutated by the usage sidecar.
    """
    return is_agent_created(skill_name)
...
def _mutate(skill_name: str, mutator, *, require_curation_eligible: bool = False) -> None:
    ...
    Usage telemetry belongs only to agent/local skills. Bundled and
    hub-installed skills are upstream-owned and stay immutable in the sidecar.
    """
    if not skill_name:
        return
    try:
