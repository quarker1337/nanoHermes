"""Compatibility shim for the relocated plugins package.

The real package lives under ``runtime/plugins`` so the repository root stays
small while legacy imports such as ``import plugins`` and ``python -m plugins.x``
...
_REAL_PACKAGE_DIR = Path(__file__).resolve().parent / "runtime" / "plugins"
